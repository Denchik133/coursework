...
public class ADFGX {
    ...
    public static String decrypt(String text, String key1, String key2) throws Exception {
        ...
        for (int h = 0; h < result.length(); h += 2){
            finalResult.append(getDecryptedChar(result.charAt(h), result.charAt(h + 1), createSquare(key1)));
        }
        return finalResult.toString();
    }

    private static char getDecryptedChar(char c, char c1, char[][] square) {
        char[] ADFGX = {'A', 'D', 'F', 'G', 'X'};
